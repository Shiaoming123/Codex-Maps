...
## 2026-08-22 — Reader 首屏采用工作区泳道，不固化三栏详情

决定：独立 Reader 的默认首页采用“概览指标 + 筛选 + 工作区泳道”的地图优先布局；Session 详情通过按需打开的抽屉呈现，列表保留为降级视图。只有获得项目/Section 的可验证来源后，才把工作区名称、计数和泳道升级为 Codex 项目。

原因：固定详情栏会削弱地图的横向关系扫视，而当前快照的 `cwd` 是已证实的事实，项目归属不是。该决定同时使原型视觉层级与 MVP 数据边界一致，避免为了还原原型而伪造数据。

补充：未筛选时地图默认显示按最近更新排序的 6 条工作线，其他工作区由显式“显示全部工作区”打开；列表视图不截断结果。这是可扫视性的默认策略，不是数据删除或归档规则。

## 2026-08-22 — 当前 Desktop 监控采用只读文件系统兼容模式

决定：在公开 Host Bridge 不可用的前提下，独立 Electron 默认监听用户本机 `~/.codex/sessions` 的 JSONL 事件文件；只投影 session 元数据和开始/完成/中断状态到地图，页面显式标记“本地兼容模式 / 非官方接口”。
